...
namespace BFi_Movie_Assignment
{
    class MainClass
    {
        public static void Main(string[] args)
        {
            ...
            string line;
            // Also decalre a list for holding the different lines
            ...
            {
                // Print that exception out
                Console.WriteLine("Exception: " + e.Message);
            }
            // Declare a variable for holding the split string 
            string[] filmInfo;
            // Add another for holding the global average 
            int globalAv = 0;
            int ukUsaAv = 0;
            int ukUsaCount = 0;
            ...
            for (int i = 2; i < 17; i++)
            {
                ...
                string filmGross = "";
                int increm = 3;
                // Here we get the last character to check if we need another index from the array
                char last = filmInfo[increm][filmInfo[increm].Length - 1];
                // Until we reach teh end of a number, keep adding the elements
                while (last != '"')
                {
                ...
                // Once we've got all the values needed from the film, add it to the global average
                globalAv += Int32.Parse(filmGross);
                // Check to see if it is UK/USA origin
                if (filmInfo[2] == "UK/USA")
                {
                    // If yes, then add on the weekend gross to the total
                    ukUsaAv += Int32.Parse(filmGross);
                    // INcrement the number of films of this origin for division later
                    ukUsaCount++;
                }
                // Now we need tp figure out how many views there were but for the first film only 
                if (firstChecked == false)
                {
                    // Divide the total gross by 8 (1 viewing = £8)
                    viewings = Int32.Parse(filmGross) / 8;
                    // We'll swap a boolean over so we only check the first iritation
                    firstChecked = true;
                }
            }
            ...
            Console.WriteLine("The average weekend gross for the top 15 films was: £{0}", globalAv.ToString("NO"));
            // Then we want to output the UK/USA origin totals
            ukUsaAv = ukUsaAv / ukUsaCount;
            Console.WriteLine("The average weekend gross for films of UK/USA origin is: £{0}", ukUsaAv.ToString("NO"));
